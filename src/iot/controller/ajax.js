...
export default class extends Base {
  ...
  async topicAction() {
      ...
      switch (order){
        ...
        break;
        case 4:
        map.register_time = {">": new Date(GetDateStr(0)+" "+"00:00:00").getTime(), "<": new Date(GetDateStr(0)+" "+"23:59:59").getTime()}
        o.update_time = 'DESC';
        break;
        case 5:
        map.register_time = {">": new Date(GetDateStr(1)+" "+"00:00:00").getTime(), "<": new Date(GetDateStr(5)+" "+"23:59:59").getTime()}
        o.update_time = 'DESC';
        break;
        case 6:
        map.register_time = {"<": new Date().getTime()}
  ...
  */
  ...
  async questionAction(){
          ...
          if(!think.isEmpty(days) && days>0){
            ...
            where = think.extend({},where,{'register_time':['>',search_time]});
          }
          //查询分类
          ...
          if(!think.isEmpty(questions.data)){
            ...
            for(let val of questions.data){
              val.register_time = await time_format(val.register_time);
              if(val.cover_id){
