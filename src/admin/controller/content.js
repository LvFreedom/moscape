...
export default class extends Base {
    ...
    async addAction(){
    	if(this.isPost()){
            ...
            if(res){
                this.success({name:"新增成功！",url:"/admin/content/index/mold/"+data.mold});
            }else {
                this.fail("更新失败！");
            }           
    	}else{
            ...
            //获取运行的文档类型
            this.active="admin/content/index";
            this.action = "/admin/content/add";
            //获取模版列表（pc）
    ...
    async editAction(){
       let category = this.model("category");
        if(this.isPost()){
            ...
            let res = await this.model("category").updates(data);
            if(res){
                this.success({name:"更新成功！",url:"/admin/content/index"});
            }else {
                this.fail("更新失败！");
            }
        }else {
          let id = this.get("cid");
           ////console.log(id);
            ...
            this.active="admin/content/index";
            this.action = "/admin/content/edit";
          ...
          //获取栏目模型信息
            ...
            if(!think.isEmpty(source.groups) && source.groups != target.groups && !think.isEmpty(target.groups)){
               ...
               await this.session("ntarget", ntarget);
                let url= `/admin/content/moveinfo/source/${data.source}/target/${data.target}`;
                if(data.merge == 1){
                   url= `/admin/content/moveinfo/merge/1/source/${data.source}/target/${data.target}`;
                }
                ...
                update_cache("category")//更新栏目缓存
                return this.success({name: "成功！",url:"/admin/content/index"})
            }
