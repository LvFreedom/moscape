...
export default class extends Base {
            ...
            for (let val of cate) {
               switch (val.mold){
                   ...
                   case 2:
                       ...
                       break;
                   default:
                       val.url = `/admin/iot/index/cate_id/${val.id}`;
               }
    ...
    async updateAction() {
        ...
        if (res) {
            ...
            if (!res.data.id) {
                ...
                this.success({name: "添加成功", url: "/admin/iot/index/cate_id/" + res.data.category_id,id:res.data.id,model_id:res.data.model_id,category_id:res.data.category_id});
            } else {
                this.success({name: "更新成功", url: "/admin/iot/index/cate_id/" + res.data.category_id,id:res.data.id,model_id:res.data.model_id,category_id:res.data.category_id});
    ...
    async saveAction() {
        ...
        if (res) {
            ...
            if (!res.data.id) {
                await this.model("action").log("add_document", "document", res.id, this.user.uid, this.ip(), this.http.url);
                this.success({name: "添加成功", url: "/admin/iot/edit/id/" + res.data.id + "/model/" + res.data.model_id +"/cate_id/"+res.data.category_id,id:res.data.id});
            } else {
 ...
 //TODO
    ...
    async countAction(){
        ...
        if(!think.isEmpty(device_id)){
            //设备开机数量
            let register_count = await this.model("document_nfc_register_event").where({device_id:device_id}).count()
            //设备读写数量
            let notify_count = await this.model("document_nfc_notify_event").where({device_id:device_id}).count()
            map.push({register_count:register_count,notify_count:notify_count})
