...
export default class extends Base {
  ...
  async indexAction() {
    ...
    if (sort) {
      ...
      console.log(typevar[0])
      this.assign("typevar",typevar);
      let tmp = {};
      let table = "type_optionvalue"+sortid;
      tmp.user_name = this.user.username;
      tmp.able_watch = ['!=', null];
      //获取电影信息
      moviedata = await this.model("document_video").join({
                  table: "document",
                  join: "left", // 有 left,right,inner 3 个值
                  as: "document",
                  on: ["id", "id"]
                ...
                as:"c",
                on:["id", "able_watch"]
              }).where(tmp).page(1,20).order("document.id DESC").countSelect();
